...
- Runtime context: a single-user Hermes gateway on a private VPS; `~/.hermes/knowledge/wb-company` contains internal business data of medium sensitivity.
- The index is derived state in profile-scoped SQLite, not authoritative memory.
- Out of scope: the broader Hermes tool execution model, Telegram authentication, WB Copilot, model-provider security, and build/release infrastructure.
