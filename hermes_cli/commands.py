...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("plan", "Toggle plan-first mode — agent shows a plan and waits for approval before executing any tools",
               "Session", args_hint="[on|off|status]",
               aliases=("echo",), subcommands=("on", "off", "status")),
    CommandDef("go", "Execute the plan shown by /plan (only needed when plan mode is on)",
               "Session", aliases=("execute",)),
...
#   - plan: plan-first mode toggle; low-frequency, reached via /hermes plan.
#   - go: executes a pending /plan; only meaningful alongside plan mode,
#     reached via /hermes go.
_SLACK_VIA_HERMES_ONLY = frozenset({"credits", "billing", "moa", "debug", "plan", "go"})
